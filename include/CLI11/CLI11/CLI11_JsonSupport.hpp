#ifndef _HG_AVIKUS_CLI11_JSONSUPPORT
#define _HG_AVIKUS_CLI11_JSONSUPPORT
...
namespace CLI
{
	// CLI11 add-on class to enable json configuration file support
	...
	private:
		...
		virtual void _to_config(const CLI::App *app, bool default_also,
								rapidjson::Value &v, rapidjson::Document::AllocatorType &alloc) const
		{
			using namespace std;
			namespace Json = rapidjson;
...
#define ELSE_IF_J_IS(type_name)   \
	...
	public:
		virtual std::vector<CLI::ConfigItem> from_config(std::istream &input) const override
		{
			using namespace std;
			namespace Json = rapidjson;
...
#endif // AVIKUS_CONFIGURATION_BY_JSON

#endif // _HG_AVIKUS_CLI11_JSONSUPPORT