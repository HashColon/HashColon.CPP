#ifndef HASHCOLON_STATISTICS
#define HASHCOLON_STATISTICS
...
namespace HashColon::Statistics
{
	...
	public:
		...
		NormalDistribution() : StatisticalDistributionBase<dimension>(),
							   _mu(Eigen::VectorR<dimension>::Zero()),
							   _Sigma(Eigen::MatrixR<dimension>::Identity()){};

		// initialize with given mu and Sigma as Identity
		NormalDistribution(const Eigen::VectorR<dimension> &mu) : StatisticalDistributionBase<dimension>(),
																  _mu(mu), _Sigma(Eigen::MatrixR<dimension>::Identity()){};

		// initialize with mu = 0 and given Sigma
		NormalDistribution(const Eigen::MatrixR<dimension> &Sigma) : StatisticalDistributionBase<dimension>(),
		...
		NormalDistribution(
			...
			const Eigen::MatrixR<dimension> &Sigma) : StatisticalDistributionBase<dimension>(),
													  _mu(mu), _Sigma(Sigma)
		{
			assert(Sigma.rows() == Sigma.cols());
		...
		{
			using namespace std;
			return pow(2 * M_PI, -0.5 * dimension) * pow(_Sigma.determinant(), -0.5) * exp(-0.5 * (x - _mu).transpose() * _Sigma.inverse() * (x - _mu));
		};
