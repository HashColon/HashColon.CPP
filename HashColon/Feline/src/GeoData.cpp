...
#include <Eigen/Eigen>
#include <Eigen/StdVector> // to use std::vector<Eigen::Matrix>
...
namespace _local
{
	...
	template <>
	GDALDataType GetGDALDataType<unsigned char>() { return GDALDataType::GDT_Byte; }
	template <>
	GDALDataType GetGDALDataType<unsigned short>() { return GDALDataType::GDT_UInt16; }
	template <>
	GDALDataType GetGDALDataType<short>() { return GDALDataType::GDT_Int16; }
	template <>
	GDALDataType GetGDALDataType<unsigned int>() { return GDALDataType::GDT_UInt32; }
	template <>
	GDALDataType GetGDALDataType<int>() { return GDALDataType::GDT_Int32; }
	template <>
	GDALDataType GetGDALDataType<float>() { return GDALDataType::GDT_Float32; }
	template <>
	GDALDataType GetGDALDataType<double>() { return GDALDataType::GDT_Float64; }
...
namespace HashColon::Feline
{
								...
								array<Real, 2> pos, ValueOption opt) const
	{
		...
		{
		case vo_Closest:
			return ValueAt(rasterIdx, _local::RoundIdx_Closest(pos));
			break;
		case vo_Interpolate:
			...
			else
			{
				...
				case GDALDataType::GDT_Byte:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](unsigned char x)
							  { return (Real)x; });
				}
				break;
				case GDALDataType::GDT_UInt16:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](unsigned short x)
							  { return (Real)x; });
				}
				break;
				case GDALDataType::GDT_Int16:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](short x)
							  { return (Real)x; });
				}
				break;
				case GDALDataType::GDT_UInt32:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](unsigned int x)
							  { return (Real)x; });
				}
				break;
				case GDALDataType::GDT_Int32:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](int x)
							  { return (Real)x; });
				}
				break;
				case GDALDataType::GDT_Float32:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](float x)
							  { return (Real)x; });
				}
				break;
				case GDALDataType::GDT_Float64:
				{
					...
					transform(tmpblk.begin(), tmpblk.end(), valblk.begin(), [](double x)
							  { return (Real)x; });
				}
				break;
				default:
	...
	void RasterGeoData::BuildCache(int rasterIdx)
	{
		...
		{
			CPLErr e = _data->GetRasterBand(rasterIdx + 1)->RasterIO(GF_Read, 0, row, xsize, 1, buffer, xsize, 1, _data->GetRasterBand(rasterIdx + 1)->GetRasterDataType(), 0, 0);
			if (!(e == 0))
			{
				throw Exception(
					"RasterGeoData: Unable to read scanline in given geospatial file");
	...
	array<size_t, 2> RasterGeoData::IndexAtPosition(array<Real, 2> pos, IndexOption opt) const
	{
		...
		{
		case idxo_Closest:
			re = _local::RoundIdx_Closest(tmp);
			break;
		case idxo_Topleft:
			re = _local::RoundIdx_TopLeft(tmp);
			break;
		default:
...
namespace HashColon::Feline
{
	...
	{
		call_once(
			SingletonRasterGeoData::_onlyOnce,
			[](size_t idx)
			{
	...
	void SingletonRasterGeoData::Initialize(const string configFilePath)
	{
		...
		clihome.add_subcommand("Feline");
		clihome.get_subcommand("Feline")->add_subcommand("Data");
		...
		cli->add_option_function<vector<string>>(
			"--cacheOnLoad", [](const vector<string> &input) -> void
			{
	...
	void SingletonRasterGeoData::Initialize_callback()
	{
		...
		{
			GetInstance()._geodata.insert({_c.namelist[i],
...
{
	template <>
	unsigned char RasterGeoData::RawValueAt<unsigned char>(size_t rasterIdx, array<size_t, 2> idx) const;
	template <>
	unsigned short RasterGeoData::RawValueAt<unsigned short>(size_t rasterIdx, array<size_t, 2> idx) const;
	template <>
	short RasterGeoData::RawValueAt<short>(size_t rasterIdx, array<size_t, 2> idx) const;
	template <>
	unsigned int RasterGeoData::RawValueAt<unsigned int>(size_t rasterIdx, array<size_t, 2> idx) const;
	template <>
	int RasterGeoData::RawValueAt<int>(size_t rasterIdx, array<size_t, 2> idx) const;
	template <>
	float RasterGeoData::RawValueAt<float>(size_t rasterIdx, array<size_t, 2> idx) const;
	template <>
	double RasterGeoData::RawValueAt<double>(size_t rasterIdx, array<size_t, 2> idx) const;
}
