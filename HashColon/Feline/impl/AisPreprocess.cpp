...
namespace HashColon::Feline
{
		...
		const shared_ptr<Table>& aisStaticTable) const
	{
		...
		if (aisStaticTable)
		{
			...
			table = InnerJoin(_table, (*aisStaticTable),
				...
				{
					return (a.at(_c.colName.imo) == b.at(_c.colName.imo))
						&& (a.at(_c.colName.mmsi) == b.at(_c.colName.mmsi));
				});
		}
		else
		{
	...
	vector<Table> AisCsvReader::ReadAisFiles(const shared_ptr<Table>& aisStaticTable) const
	{
		...
		vector<Table> re; re.reserve(filenames.size());
	...
	{	
		table.erase( remove_if(
			...
			[&](const Row& row)->bool {
				...
				if (!all_of(imoStr.begin(), imoStr.end(), [](char c) {return isdigit(c) != 0; }) 
					|| (imoStr.size() > 7) || stoi(imoStr) <= 0)
					return true;
				if (!all_of(mmsiStr.begin(), mmsiStr.end(), [](char c) {return isdigit(c) != 0; })
	...
	void AisCsvReader::RemoveJumpAndAnchorPoints(Table& table) const
	{
		...
		for (size_t i = 1; i < table.size(); i++)
		{
			...
			TimePoint currTp; currTp.fromString(table[i][_c.colName.timestamp].Get(), _c.timeFormatStr);
			Real dist = lastPos.DistanceTo(currPos);
			Real vel = dist / (Real)(chrono::duration_cast<chrono::hours>(currTp - lastTp).count());
			if ((vel < _c.jumpingCriteria) && (vel > _c.anchorCriteria)
				&& (dist > _c.anchorDistCriteria))
			{
				lastPos = currPos;
...
namespace HashColon::Feline
{
		...
		const vector<Table> refinedAisCsv) const
	{
		...
		TimePoint criteriaTP; criteriaTP.fromString(_c.timeIntervalCriteria, _c.timeFormatStr);
