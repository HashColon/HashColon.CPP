...
using namespace HashColon::Feline::Data;
...
namespace HashColon::Feline
{
	...
	{
		return XTD(beam, beam);
	}

	XTD XTDEstimation::_Get_d_pos() const
	{
		return XTD(_c.PositionPrecision, _c.PositionPrecision);
	}
	...
	{
		struct itemtype { std::array<size_t, 2> idx; XTD xtd; };
		struct XtdCompare {
		...
		XTD getxtd_prim(Position A, Position B, array<Real, 2> P_arr)
		{
			...
			{
				return XTD(A.DistanceTo(P), A.DistanceTo(P));
			}
			...
			if (projlength < 0)
				return xtd_val > 0 ? XTD(A.DistanceTo(P), 0) : XTD(0, A.DistanceTo(P));
			else if (projlength > 1)
				return xtd_val > 0 ? XTD(B.DistanceTo(P), 0) : XTD(0, B.DistanceTo(P));
			else
		...
		Position pos, Position legEnd, XTD searchLimit, Real beam, Real draught) const
	{
		...
		// Bathymetry data
		Data::RasterGeoData::Ptr gdata = Data::SingletonRasterGeoData::GetInstance(_threadIdx).GetData("Bathymetry");		
		
		// initialize visit checker as falase
		auto mapsize = gdata->GetRasterSize();
		vector<vector<bool>> vc; vc.resize(mapsize[0]);
		...
		auto initIdx1 = gdata->IndexAtPosition({ pos[0], pos[1] }, Data::RasterGeoData::idxo_Closest);
		auto initPos1 = gdata->PositionAtIndex(initIdx1);
		auto initIdx2 = gdata->IndexAtPosition({ legEnd[0], legEnd[1] }, Data::RasterGeoData::idxo_Closest);
		...
		do {
			// pop current
			itemtype current = queue.top(); queue.pop();
			...
			if (gdata->ValueAt(0, current.idx) > -(_c.DraughtMargin_D_coast + draught))
			{
				...
				{					
					...
					// 원래는 grounding check을 하고 gounding route에 대해 오류를 내야 하지만, 
					// 데이터 정밀도의 한계로 그냥 최소값 출력함. 
			...
			else
			{
				...
				for (size_t i = 0; i < 4; i++)
				{
					...
					if (newIdx[0] >= mapsize[0] || newIdx[1] >= mapsize[1]) continue;

					// check if visited
					if (vc[newIdx[0]][newIdx[1]]) continue;
					else vc[newIdx[0]][newIdx[1]] = true;
					...
					if (newXtd[0] > maxlimit[0] || newXtd[1] > maxlimit[1]) continue;
	...
	void XTDEstimation::Initialize(const string configFilePath)
	{
		...
		cli->add_option_function<vector<double>>("--NavigationalSafetyAllowanceTable", 
			[](const vector<double>& input) -> void
			{
				size_t cnt = 0; _cDefault.NavigationalSafetyAllowanceTable.resize(3);
